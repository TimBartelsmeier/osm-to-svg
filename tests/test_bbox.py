...
def test_symmetric_bbox_is_centred_on_coordinates() -> None:
    lat, lon = 52.5, 9.7
    polygon = get_bbox_around_coordinates(lat, lon, width_km=10, height_km=20)
...
def test_symmetric_bbox_lon_approx_values() -> None:
    ...
    polygon = get_bbox_around_coordinates(52.5, 9.7, width_km=10, height_km=20)
    longitudes = [point[1] for point in polygon]

    assert min(longitudes) == pytest.approx(expected_min_lon, rel=1e-6)
    assert max(longitudes) == pytest.approx(expected_max_lon, rel=1e-6)
...
def test_symmetric_bbox_lat_approx_values() -> None:
    ...
    assert min(latitudes) == pytest.approx(52.5 - 10 / 111.0, rel=1e-4)
    assert max(latitudes) == pytest.approx(52.5 + 10 / 111.0, rel=1e-4)
...
def test_asymmetric_bbox_east_west() -> None:
    ...
    polygon = get_bbox_around_coordinates(
        lat, lon, east_km=11.1, west_km=22.2, north_km=5, south_km=5
    )
    longitudes = [point[1] for point in polygon]

    assert min(longitudes) == pytest.approx(-22.2 / 111.0, rel=1e-4)
    assert max(longitudes) == pytest.approx(11.1 / 111.0, rel=1e-4)
...
def test_asymmetric_bbox_north_south() -> None:
    ...
    assert min(latitudes) == pytest.approx(-11.1 / 111.0, rel=1e-4)
    assert max(latitudes) == pytest.approx(33.3 / 111.0, rel=1e-4)


def test_bbox_ordering() -> None:
    """south/west values are always less than north/east values."""
    polygon = get_bbox_around_coordinates(48.137, 11.576, width_km=15, height_km=10)
    latitudes = [point[0] for point in polygon]
    longitudes = [point[1] for point in polygon]

    assert min(longitudes) < max(longitudes)
    assert min(latitudes) < max(latitudes)
