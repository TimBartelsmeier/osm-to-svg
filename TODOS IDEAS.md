Quality Control
---------------
- geocode: separate place query and bbox query to allow querying POIs
- Make lat/lon order consistent (bbox / pois)
- - Check readme for errors & missing info
- Check overpass functionality - (code broken or was it just overloaded everytime I checked?)
- Document/change behaviour for download methods and image saving methods for cases like missing parent dir, file already exists, etc.

Feature Ideas
-------------
- Other map orientations and projections

For Release
-------
- Check consistent/updated library name, update github urls or similar in code
- AI disclaimer
