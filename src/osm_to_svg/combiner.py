...
def combine_svgs(svg_paths: list[str], output_path: str) -> None:
    """Combine multiple SVG files into a single layered SVG.

    The order of svg_paths determines the z-order in the final image
    (first file is bottom layer, last file is top layer).
    ...
    Raises:
        ValueError: If svg_paths is empty or files have inconsistent dimensions
    """
    if not svg_paths:
        raise ValueError("svg_paths cannot be empty")

    # Parse first SVG to get base dimensions and viewBox
    first_tree = ET.parse(svg_paths[0])
    first_root = first_tree.getroot()

    # Extract dimensions and viewBox from first SVG
    width = first_root.get("width")
    height = first_root.get("height")
    viewbox = first_root.get("viewBox")
    ...
    for idx, svg_path in enumerate(svg_paths):
        ...
        _append_layer_from_source(
            ...
            viewbox=viewbox,
            layer_id=f"layer-{idx}",
            source_name=f"SVG file {svg_path}",
        )
